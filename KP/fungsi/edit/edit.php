...
if(!empty($_SESSION['admin'])){
	...
	if(!empty($_GET['stok'])){
		...
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index.php?page=barang/tambah&barang='.$id. '&success-stok=stok-data"</script>';
	}
	if(!empty($_GET['restok'])){
		$restok = htmlentities($_POST['restok']);
		$id = htmlentities($_POST['id']);
		$dataS[] = $id;
		$sqlS = 'select*from barang WHERE id_barang=?';
		...
		$row = $config -> prepare($sql);
		$row -> execute($data);
		echo '<script>window.location="../../index.php?page=barang&barang='.$id. '&success-stok=stok-data"</script>';
	}
	...
	if(!empty($_GET['member'])){
		$id = htmlentities($_POST['id_member']);
		...
		$sql = 'UPDATE member SET username=?, pass=?,
				nm_member=?, alamat_member=?, telepon=?, email=?, NIK=?  WHERE id_member=?';
		$row = $config -> prepare($sql);
		$row -> execute($data);
		
		echo '<script>window.location="../../index.php?page=user/edit&member='.$id.'&success=edit-data"</script>';
	}
		...
		<?php foreach($hasil1 as $hasil){?>
			<tr>
				...
				<a href="fungsi/tambah/tambah.php?jual=jual&id=<?php echo $hasil['id_barang'];?>&id_kasir=<?php echo $_SESSION['admin']['id_member'];?>" 
					...
					abc<i class="fa fa-shopping-cart"></i></a></td> -->
			</tr>
		<?php }?>
		</table>
<?php	
