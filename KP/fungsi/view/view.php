	 ...
	 */ 
	 class view {
		...
		function __construct($db){
			...
			function laporan($id){
				$sql = "select nota.id_nota, barang.id_barang, barang.nama_barang, nota.jumlah, nota.total
						from nota inner join barang on nota.id_barang = barang.id_barang
						where nota.id_nota=?";
				$laporan = $this-> db -> prepare($sql);
				$laporan->bindParam(1, $id);
				$laporan -> execute();
			...
			function lihatidnota(){
				$sql = 'SELECT id_nota FROM nota ORDER BY id DESC LIMIT 1';
				$row = $this-> db -> prepare($sql);
				$row -> execute();
				$hasil5 = $row -> fetch();
				return $hasil5;
			...
			function periode_jualprint($periode){
				$sql ="SELECT nota.* , barang.id_barang, barang.nama_barang, barang.harga_beli, member.id_member,
						member.nm_member from nota 
					   left join barang on barang.id_barang=nota.id_barang 
					   left join member on member.id_member=nota.id_member WHERE nota.periode = ? 
